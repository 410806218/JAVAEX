...
import tankwariii.TankWar;
import tankwariii.Tank;
import tankwariii.Missile;
import tankwariii.GameObject;
import tankwariii.EnemyTank;
...
public class WarField extends GameObject {
	...
	public void restart() {
		...
		for (int j = 0; j < 4; j++) {
                        for (int i = 0; i < 12; i++) {
			this.enemyTanks.get(i + 12 * j).setX(30 + i * 60);
			this.enemyTanks.get(i + 12 * j).setY(70 + j * 60);
			this.enemyTanks.get(i + 12 * j).setLive(true);
                    }
                }
	...
	public WarField(int x, int y, int width, int height, Color color) {
		...
		// TODO Auto-generated constructor stub
		//this.setBackground(Color.GREEN);
		ourTank = new Tank(this, 355, 600, 100, 5, Color.YELLOW);
		for (int j = 0; j < 4; j++) {
                    for (int i = 0; i < 25; i++) {
			this.enemyTanks.add(new EnemyTank(this, 30 + i * 60, 70 + j * 60, 50, 50,
					Color.WHITE));
                    }
		}
                //ball = 
	...
	void draw(Graphics g) {
		...
		g.drawString("" + (48-numberOfEnemyTanks), 600, 20);
			...
			for (Tank enemyTank : this.enemyTanks) {
				if (enemyTank.isLive())
					m.hitTank(enemyTank);
                        ...
                        if(m.hitTank(ourTank)) m.reBound(8);
		}
